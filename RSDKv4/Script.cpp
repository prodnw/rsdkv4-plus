...
ScriptVariableInfo scriptValueList[SCRIPT_VAR_COUNT] = {
    ...
    ScriptVariableInfo(VAR_ALIAS, ACCESS_PUBLIC, "MENU_5", "4"),
    ScriptVariableInfo(VAR_ALIAS, ACCESS_PUBLIC, "MENU_6", "5"),
    ScriptVariableInfo(VAR_ALIAS, ACCESS_PUBLIC, "MENU_7", "6"),
    ScriptVariableInfo(VAR_ALIAS, ACCESS_PUBLIC, "MENU_8", "7"),
    ...
    while (running) {
            ...
            if (opcodeType == SCRIPTVAR_VAR) {
                ...
                switch (scriptCode[scriptCodePtr++]) {
                    ...
                    case VAR_SYSTEM_TIMEMONTH: {
                        time_t now = time(NULL);
                        struct tm *tm_now = localtime(&now);
                        scriptEng.operands[i] = tm_now->tm_mon;
        ...
        switch (opcode) {
            ...
            case FUNC_GETTEXTINFO: {
                // no going out of bounds!!
                if (scriptEng.operands[1] < 0)
                    scriptEng.operands[1] = 0;
                if (scriptEng.operands[1] > TEXTMENU_COUNT)
                    scriptEng.operands[1] = TEXTMENU_COUNT - 1;
...
//-------- New functions start here --------//
            ...
            case FUNC_DRAWSTRING: {
               ...
               // int charID = 0;
                ...
                if (scriptEng.operands[6] > TEXTMENU_COUNT)
                    scriptEng.operands[6] = TEXTMENU_COUNT - 1;
